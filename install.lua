...
local RunBot = io.open("RAWAN", 'w')
RunBot:write([[
#!/usr/bin/env bash
cd $HOME/RAWAN
token="]]..database:get(Server_Done.."Token_Write")..[["
rm -fr RAWAN.lua
wget "https://raw.githubusercontent.com/RAWANTEAM/RAWAN/main/RAWAN.lua"
while(true) do
rm -fr ../.telegram-cli
./tg -s ./RAWAN.lua -p PROFILE --bot=$token
done
]])
RunBot:close()
...
while(true) do
rm -fr ../.telegram-cli
screen -S RAWAN -X kill
screen -S RAWAN ./RAWAN
done
]])
RunTs:close()
end
